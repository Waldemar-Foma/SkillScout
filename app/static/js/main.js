document.addEventListener('DOMContentLoaded', function() {
    ...
    async function loadWorkJoke() {
        ...
        try {
            funFactText.textContent = "Интересный факт о работе:";
            
            // Используем API для получения случайного анекдота (пример)
            const response = await fetch('https://official-joke-api.appspot.com/jokes/programming/random');
            const data = await response.json();
            
            if (data && data[0]) {
                jokeElement.innerHTML = `
                    <p><strong>${data[0].setup}</strong></p>
                    <p>${data[0].punchline}</p>
                `;
            } else {
                // Запасной вариант, если API не сработает
                jokeElement.innerHTML = `
                    <p><strong>Почему программисты путают Хэллоуин и Рождество?</strong></p>
                    <p>Потому что Oct 31 == Dec 25!</p>
                `;
            }
        } catch (error) {
            console.error('Ошибка при загрузке анекдота:', error);
            jokeElement.innerHTML = `
                <p><strong>Как объяснить разницу между Java и JavaScript?</strong></p>
                <p>Это как разница между машиной и ковриком для мыши.</p>
            `;
        }
    }
...
function getMBTIDescription(type) {
    const descriptions = {
        'INTJ': { 
            title: 'Стратег (INTJ)', 
            desc: 'Аналитичный, решительный, независимый мыслитель с богатым воображением и стратегическим мышлением.', 
            strengths: 'Стратегическое мышление, уверенность в себе, целеустремленность, логичность.', 
            weaknesses: 'Может быть высокомерным, слишком критичным, нетерпеливым к некомпетентности.' 
        },
        'INTP': { 
            title: 'Логик (INTP)', 
            desc: 'Изобретательный, логичный и любознательный аналитик с неутолимой жаждой знаний.', 
            strengths: 'Аналитический ум, креативность, объективность, любовь к решению сложных задач.', 
            weaknesses: 'Может быть непрактичным, рассеянным, излишне скептичным и неэмоциональным.' 
        },
        'ENTJ': { 
            title: 'Командир (ENTJ)', 
            desc: 'Решительный, харизматичный и уверенный в себе лидер, который любит принимать сложные решения.', 
            strengths: 'Лидерские качества, стратегическое мышление, эффективность, прямота.', 
            weaknesses: 'Может быть нетерпеливым, агрессивным, излишне доминирующим и нечувствительным.' 
        },
        'ENTP': { 
            title: 'Полемист (ENTP)', 
            desc: 'Умный, любознательный и изобретательный мыслитель, который любит интеллектуальные вызовы.', 
            strengths: 'Быстрое мышление, креативность, остроумие, умение видеть возможности.', 
            weaknesses: 'Может быть несосредоточенным, нетерпеливым, склонным к спорам и противоречиям.' 
        }
    };
    return descriptions[type] || null;
}

// Показ подсказки с информацией о MBTI-типе
